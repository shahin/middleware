...
UMOIQ_URL_TMPL = "https://webservices.umoiq.com/api/pub/v1/agencies/{agency}/stopcodes/{stop_code}/predictions?key={umoiq_api_key}"  # noqa: E501
UMOIQ_API_KEY = os.getenv("UMOIQ_API_KEY")


computed_fields = {
    "agency_id": lambda preds, pred: preds["agency"]["id"],
    "response_date": lambda preds, pred: datetime.fromtimestamp(
        preds["serverTimestamp"] / 1000.0
    ).strftime("%Y%m%d"),
    "response_time": lambda preds, pred: datetime.fromtimestamp(
        preds["serverTimestamp"] / 1000.0
    ).strftime("%H:%M:%S"),
    ...
    "arrival_seconds": lambda preds, pred: round(
        (
            datetime.fromtimestamp(pred["timestamp"] / 1000.0) - datetime.now()
        ).total_seconds()
    ),
...
def parse_predictions(response_data: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for _predictions in response_data:
        ...
        for _prediction in _predictions["values"]:
            prediction = {
                k: v(_predictions, _prediction) for k, v in computed_fields.items()
            }
            predictions.append(prediction)
    return sorted(
        predictions,
        key=lambda pred: (pred["route_id"], pred["direction_name"], pred["vehicle_id"]),
    )
...
def main(
    ...
    umoiq_url = UMOIQ_URL_TMPL.format(
        stop_code=stop_code, umoiq_api_key=UMOIQ_API_KEY, agency=agency
    )
    ...
    if response_data_path is None:
        response_data_path = (
            f"responses_{datetime.now().strftime('%Y%m%dT%H%M%S')}_{agency}.json"
        )
    ...
    while True:
        ...
        if not resp.ok:
            ...
            predictions_w_sorted_keys = [
                {k: prediction[k] for k in field_order} for prediction in predictions
            ]
